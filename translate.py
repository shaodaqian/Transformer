...
def translation_score(pred_seq, ends, gold, TRG):
    ...
    for i in range(len(pred_seq)):
        ...
        pred_line = pred_line.replace(special_tokens.BOS_WORD, '').replace(special_tokens.EOS_WORD, '').replace(
            special_tokens.PAD_WORD, '')
        target = gold[i]
        target_line = ' '.join(TRG.vocab.itos[idx] for idx in target)
        target_line = target_line.replace(special_tokens.BOS_WORD, '').replace(special_tokens.EOS_WORD, '').replace(
            special_tokens.PAD_WORD, '')
...
def main():
    ...
    bleu_score = 0
    for example in tqdm(
        test_loader, mininterval=20, desc='  - (Test)',
        leave=False, total=total_tokens // args.batch_size
    ):
